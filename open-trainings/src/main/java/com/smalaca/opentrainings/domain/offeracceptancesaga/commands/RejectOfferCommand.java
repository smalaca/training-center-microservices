...
public record RejectOfferCommand(CommandId commandId, UUID offerId, String reason) implements OfferAcceptanceSagaCommand {
    ...
    public static RejectOfferCommand nextAfter(NoAvailableTrainingPlacesLeftEvent event) {
        return new RejectOfferCommand(event.eventId().nextCommandId(), event.offerId(), "No available training places left");
    }
}
